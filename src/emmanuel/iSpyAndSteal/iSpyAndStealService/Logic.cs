...
public class Logic
{
    ...
    public static V_USER_LOGIN_DETAIL AuthenticateUser(string userid, string password, string mac)
    {
        using (var db = new iSpyDb())
        {
            return db.V_USER_LOGIN_DETAIL.Where(x =>  x.EMAIL == userid && x.PASSWORD==password && x.MAC_ADDRESS==mac).FirstOrDefault();
        }
    }
    ...
    internal static List<ListItem> GetAllSystem()
    {
        using (var db = new iSpyDb())
            return db.SYSTEM_INFORMATION.ToList().Select(s => new ListItem { Text = s.SYSTEM_ID.ToString(), Value = s.MAC_ADDRESS }).ToList();
    }
}